...
class FourTypesChartDrawer(private val purityValues: Array<Double>) :
    ChartDrawer("Coevolution/selection method results") {
    private val labels = arrayOf(
        "No coevolution, Hv-Pareto",
        "No coevolution, NSGA-II",
        "Coevolution, Hv-Pareto",
        "Coevolution, NSGA-II"
    )
    ...
    override fun setSeriesRendering(chart: JFreeChart, plot: XYPlot) {
        val chartinfo = TextTitle(buildString {
            for (i in 0 until 4) {
                append(
                    "${labels[i]} - Purity: ${"%.3f".format(purityValues[i])}, Front size: ${algorithmOutcomes!![i].size}, " +
                            "Hypervolume: ${
                                "%.3f".format(
